...
def test_i2s_clocks():
    ...
    async def testbench(ctx):
        ...
        for _ in range(s_w_ratio-1):
            await ctx.tick(domain='mclk').repeat(2*m_s_ratio*s_w_ratio+1)
            assert ctx.get(i2s_clocks.ws) == 1
            await ctx.tick(domain='mclk').repeat(2*m_s_ratio*s_w_ratio)
        await ctx.tick(domain='mclk').repeat(m_s_ratio*10)
